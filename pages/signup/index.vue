    ...
    <div class="card-body">
      ...
      <form @submit.prevent="signupWithCredentials">
        ...
        <div class="password-validation">
          ...
          <div class="check">
            <ShieldCheckIcon class="icon" :class="{'success': validation.alphanumerical}"/>
            <p :class="{'success': validation.alphanumerical}">
              {{ $t('Contains alphanumerical character') }}
            </p>
          </div>
          <div class="check">
            <ShieldCheckIcon class="icon" :class="{'success': validation.special}"/>
            <p :class="{'success': validation.special}">
              {{ $t('Contains special character') }}
            </p>
          </div>
          <div class="check">
            <ShieldCheckIcon class="icon" :class="{'success': validation.uppercase}"/>
            <p :class="{'success': validation.uppercase}">
              {{ $t('Contains uppercase character') }}
            </p>
          </div>
          <div class="check">
            <ShieldCheckIcon class="icon" :class="{'success': validation.minLength}"/>
        ...
        <PasswordField v-model="passwordRetype" :label="$t('Retype Password')"/>

        <div class="password-validation">
          <div class="check">
            <ShieldCheckIcon class="icon" :class="{'success': validation.match}"/>
        ...
        <button type="submit" class="btn btn-success w-full mt-2" :disabled="!validation.canSubmit()">
          {{ $t('Sign up') }}
        </button>
      </form>

      <div class="separator mt-7 mb-7">{{ $t('Already have an account?') }}</div>

      <nuxt-link-locale to="/login">
        <button class="btn btn-secondary btn-outline text-secondary-content w-full">{{ $t('Login') }}</button>
      </nuxt-link-locale>
    </div>
  </div>
</template>
...
async function signupWithCredentials() {
  ...
  try {
    const user = await createUserWithEmailAndPassword(<Auth>auth, email.value.trim(), password.value);
